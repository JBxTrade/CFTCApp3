...
export class LinkUpdatePage {
  ...
  theLinkInput = element(by.id('field_theLink'));
  ...
  async setTheLinkInput(theLink: string): Promise<void> {
    await this.theLinkInput.sendKeys(theLink);
  }

  async getTheLinkInput(): Promise<string> {
    return await this.theLinkInput.getAttribute('value');
  }
